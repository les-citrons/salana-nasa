...
class utilities(commands.Cog):
    ...
    async def help(self, ctx, cmd=None):
        ...
        if cmd == None:
            ...
            extra_msg.add_field(name='__NUMBER COLLECTING__', value='Numbers that you generate with ,random are placed in your number collection! Use `,numbers` to see what numbers are in your collection. You can also group numbers into named collections. Use `,collections` to see all of your collections. Use `,newcollection <name>` to create a collection and `,movenumber <destination> number [source]` to move a number into a collection. If you want to remove a collection, you can use `delcollection <name>`. You can also give other people numbers in your collection with `,give <someone> <number> [collection]`.', inline = False)
            extra_msg.add_field(name='__ECONOMY__', value='mani li lon. Each person is given an initial balance of 100. Use `,balance` to view your current balance. Use `,pay <someone> <amount>` to give people money. Use `,recycle <number> [collection]` to recycle a number and recieve a small reward. If you have generated a digit with 5 or more digits, you can sell the number with `,sell <number> [collection]`.', inline = False)
            extra_msg.add_field(name='__SEEDING AND BETTING__', value='With `,bet <amount>` you can bet a certain amount of mani on beating the high score with `,random`. Seeds make `,random` a lot less random. With seeds, you can control the behavior of the random number generator. Use `,seedprice` to view the current seedprice. Then you can use `,buyseeds` to purchase a seed. Use `,seeds` to view how many seeds you have. Once you have a seed, you can use it to seed the random number generator. To use a seed, simply supply a seed to `,random`, for instance, if to seed the random generator with the string "foo", use `,random foo`.', inline = False)
